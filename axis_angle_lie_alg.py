...
from pytorch3d.transforms import so3_exp_map, so3_log_map, matrix_to_axis_angle, axis_angle_to_matrix
...
class AngleAxisModel(nn.Module):
    def __init__(self, mesh, renderer, gt_image, R_init, T_init, device):
        ...
        # Lie algebra initialization using logarithm map
        # so3_log_map: SO(3) -> R^3 (axis-angle representation)
        self.R = nn.Parameter(so3_log_map(R_init))
        self.T = nn.Parameter(T_init)

    def forward(self):
        # Map Lie algebra back to SO(3)
        R = so3_exp_map(self.R)
        T = self.T
...
def train_angle_axis_model(mesh, gt_image, R_init, T_init, silhouette_renderer, phong_renderer, object_name, n_epochs=500, device="cuda:0"):
    ...
    for i in tqdm.tqdm(range(n_epochs)):
        ...
        if loss < min_loss:
            ...
            R_min = so3_exp_map(model.R.detach())
            T_min = model.T
            min_loss = loss
